...
// Retrieve the votes
async function voteList(Dao1901Votes, voteId) {
  var votes = []
  var addr = (await Dao1901Votes.proposals(voteId))[2] // vote list head
  while (addr != 0) {
    var v = await Dao1901Votes.getVote(voteId, addr)
    votes.push(v[0]);
    addr = v[1];
  }
  return votes
...
contract('Dao1901Votes', (accounts) => {
  ...
  contract('Voting', () => {
    ...
    it('should have 2 votes', async () => {
      ...
      let votes = await voteList(Dao1901Votes, 1)
