...
function* bootstrap() {
  ...
  yield put({type: 'ETHEREUM_INFO_REQUESTED'})
  yield put({type: 'CONTRACTS_INFO_REQUESTED'})
  const { error } = yield race({
    success: take('CONTRACTS_INFO_SUCCEED'),
    error: take('CONTRACTS_INFO_FAILED'),
  })
  if (error) throw new Error(error)
  yield put({type: 'OWNER_ADDRESS_REQUESTED'})
