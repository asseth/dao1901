...
function runMemberTests() {
    ...
    assert(!daoMembers.isMember(bob), "Bob is a DAO member before subscribing");

    // subscribe account 1 for 1 year
    console.log('Owner adds a member...');
    daoMembers.subscribe.sendTransaction(bob, 1, {from:alice});
    admin.sleepBlocks(3);
    assert(daoMembers.isMember(bob), "Bob is not a DAO member after subscribing");
    ...
    admin.sleepBlocks(3);
    assert(!daoMembers.isMember(bob), "Bob is still a member after revokation");
    ...
    admin.sleepBlocks(3);
    assert(daoMembers.isMember(bob), "Bob is not a DAO member after renewal");
    assert(daoMembers.isMember(carol), "Carol is not a DAO member after subscribing");
    ...
    admin.sleepBlocks(3);
    assert(!daoMembers.isMember(carol), "non-owners was able to add a member");
